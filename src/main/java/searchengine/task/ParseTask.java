...
public class ParseTask extends RecursiveAction {
    private final static int PAUSE_DURATION = 500;
    private final static int MEDIUMTEXT_LENGTH = 16777215;
    private final static Logger logger = LogManager.getLogger(ParseTask.class);
    private final static String CSS_QUERY = "a";
    private final static String HASH_TEXT = "#";
    private final static String QUESTION_TEXT = "?";
    private final static char SLASH_SYMBOL = '/';
    private final static char CHAR_SERVER_ERROR = '5';
    private final static char CHAR_CLIENT_ERROR = '4';
            ...
            for (Attribute attr : element.attributes()) {
                if (isCorrectAttr(attr.getValue())) {
                    ...
                    if (isCorrectLink(ref) && (foundedPages.isEmpty()) && (!(common.getIsInterrupt()))) {
                        ...
                        Connection connection = common.getConnection(newPage);
    ...
    @Transactional
    private void renewDateInDB(Page newPage) {
        try {
            ...
            if (!(isErrorPage(newPage))) {
                LemmaTask lemmaTask = common.createLemmaTask();
                lemmaTask.setPage(newPage);
                lemmaTask.run();
                lemmaTask.saveLemmas();
            }
        } catch (Exception ex) {
